...
inlineAssignFlatten :: Int
                    -> Module a c b
                    -> Either (Error ()) (Module () (ConsAnn (StackType ())) (StackType ()), (Int, ConsSizes))
...
monomorphize :: Int
             -> Module a c b
             -> Either (Error ()) (Module () (ConsAnn MonoStackType) MonoStackType, ConsSizes)
monomorphize ctx m = do
    (flat, (_, cs)) <- inlineAssignFlatten ctx m
    let flatFn' = filter (not . isTyDecl) flat
    (, cs) <$> traverse (bitraverse tryMonoConsAnn tryMono) flatFn'
